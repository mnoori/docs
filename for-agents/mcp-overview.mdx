...
description: "Patchline exposes music-business tools via the Model Context Protocol (MCP) at www.patchline.ai/api/mcp/v1. Connect any MCP client — Cursor, Codex, Claude Desktop, Claude Code, VS Code — and your AI gains catalog, artist intelligence, playlist matching, and smart link tools."
icon: "code"
keywords:
  - patchline mcp
  - mcp server music
...
schema:
  - "@context": "https://schema.org"
    ...
    "description": "MCP server for music industry workflows. OAuth 2.0 with PKCE."
    "offers":
      "@type": "Offer"
      "price": "0"
      "priceCurrency": "USD"
...
Grouped by category. This page documents the stable public tool families.
For the exact current tool list, connect an MCP client and run
`tools/list`.
...
The tools above are the stable, most-used surface. The MCP server also
exposes additional tool families that evolve with the product:
...
For the exact, current list with full schemas, **connect an MCP client
and run `tools/list`**. That response is the canonical source of truth
...
If you're an AI agent reading this page programmatically, use these
rules of engagement and confirm exact schemas with MCP `tools/list`:
...
- **Latency:** Most tools return in under 500ms. Tools that refresh
  provider-backed data or run analysis can take 2–5s for cold-cache
  calls. Cached results are under 200ms.
